...
대도시 서울에서는 하루 수백만 명이 지하철을 이용하며,  
지하철 이용량의 변화는 **열차 배차, 혼잡 관리, 안전 인력 배치** 등  
도시 교통 운영 전반에 큰 영향을 미칩니다.

일반적으로 지하철은 버스나 자가용과 달리  
**날씨의 영향을 상대적으로 덜 받을 것이라는 가설**이 존재하지만,  
이를 **실제 데이터로 검증한 연구는 충분하지 않습니다.**

본 프로젝트는 **2015~2025년 서울시 월별 강수량 데이터와  
지하철 승·하차 인원 데이터를 결합**하여  
강수량이 지하철 이용량에 미치는 영향을  
데이터 기반으로 분석하는 것을 목표로 합니다.
""")
